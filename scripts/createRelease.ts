...
const thisReleaseVersion = thisRelease?.version ?? "";
if (!thisRelease || !thisReleaseVersion) throw new TypeError("No versioned release was found.");

// Handy info
const tag: string = `v${thisReleaseVersion}`;
...
const prereleaseLength = thisRelease.parsedVersion?.prerelease.length ?? 0;
const isPrerelease: boolean = prereleaseLength > 0 || thisRelease.parsedVersion?.major === 0;
